...
(() => {
  ...
  function renderAuth(){
    if(user&&authMode!=='reset'){dialog.innerHTML=`<div class="bc-head"><h2 id="bc-auth-title">내 계정</h2><button class="bc-close" data-close aria-label="닫기">×</button></div><p class="bc-note" translate="no">${esc(user.email)}</p><p class="bc-auth-message" id="bc-auth-message" role="status"></p><div class="bc-stack" style="margin-top:22px"><button class="bc-btn" id="bc-reload">계정 기록 다시 불러오기</button><button class="bc-btn" id="bc-import">이 기기의 방문자 기록 가져오기</button><button class="bc-btn" id="bc-export">내 기록 내려받기</button><button class="bc-btn" id="bc-signout">로그아웃</button></div><p class="bc-note bc-modal-foot">네 사이트에서 같은 이메일과 비밀번호를 사용할 수 있습니다. 사이트 주소가 다르면 각각 로그인해 주세요.</p>`;
  ...
  function gangwon(){
    const photos=window.MA_WORKSPACE_PHOTOS;
    const regions=$$('#regions .index-list li').map((el,i)=>({el,id:`region-${i}`,name:el.dataset.regionName||$('b',el)?.textContent||'',description:el.dataset.regionDescription||$('span',el)?.textContent||'',lenses:el.dataset.lenses||''}));
    function generatedTitle(journey) {
      if (journey.customTitle || typeof journey.title !== 'string' || !Array.isArray(journey.days)) return false;
      const name = regions.find(region => region.name === journey.title.split(' ')[0])?.name;
      return !!name && journey.title === `${name} ${journey.days.length === 1 ? '하루' : journey.days.length + '일'}의 기록`;
    }
    panel.innerHTML=`<div class="bc-collection-layout"><section class="bc-card"><div class="bc-collection-heading"><h3>담아둔 지역 <span id="bc-region-saved-count">0</span></h3><a class="bc-btn" href="#regions">지역 둘러보기</a></div><ul class="bc-list" id="bc-region-saved"></ul></section><section class="bc-card"><h3>저장한 여행</h3><div id="bc-journey-photo"></div><p class="bc-note" id="bc-journey-summary" translate="no"></p><a class="bc-btn" href="#journey">여행 계획 열기</a></section></div><details class="bc-design-settings"><summary>저장한 디자인 실험</summary><p class="bc-note">풍경 기록의 실험 설정을 이어서 사용할 수 있습니다.</p><div class="bc-row"><a class="bc-btn" href="#lab">실험 열기</a><button class="bc-btn" id="bc-save-design">현재 설정 저장</button><button class="bc-btn" id="bc-load-design">설정 불러오기</button></div></details>`;
    ...
    renderFeature=()=>{const selected=regions.filter(r=>state.bookmarks.includes(r.id));$('#bc-region-saved-count').textContent=selected.length;$('#bc-journey-summary').setAttribute('translate',state.journey&&!generatedTitle(state.journey)?'no':'yes');$('#bc-journey-summary').textContent=state.journey?.title||'저장한 여행이 아직 없습니다.';$('#bc-journey-photo').innerHTML=state.journey?photos?.figure(state.journey.region,'bc-saved-trip-photo')||'':'';$('#bc-region-saved').innerHTML=selected.length?selected.map(r=>`<li class="bc-saved-region"><div class="bc-saved-region-main"><a class="bc-saved-region-link" href="#region-${regions.indexOf(r)}">${photos?.thumbnail(r.name)||''}<span>${esc(r.name)}</span></a>${photos?.sourceLink(r.name)||''}</div><button class="bc-btn bc-chip" data-bookmark="${r.id}" aria-label="${esc(r.name)} 즐겨찾기 해제">해제</button></li>`).join(''):`<li class="bc-saved-empty">${photos?.figure('춘천','bc-empty-photo')||''}<p class="bc-note">어떤 풍경을 담고 싶으세요? 지역의 ‘담기’를 누르면 사진과 함께 모아볼 수 있어요.</p></li>`;regions.forEach(r=>{const b=$('[data-bookmark]',r.el);b.textContent=state.bookmarks.includes(r.id)?'담음 ✓':'담기';b.setAttribute('aria-pressed',String(state.bookmarks.includes(r.id)));});};
  }
  function blog(){
